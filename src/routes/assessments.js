...
async function generateReportInBackground({ assessmentId, handler, userEmail, userName }) {
  ...
  } catch (reportErr) { console.error('Background report generation error:', reportErr); }
}
...
userAssessmentRouter.post('/', async (req, res) => {
  try {
    ...
    let toolId = null;
    if (toolSlug) { const tool = await prisma.tool.findUnique({ where: { slug: toolSlug } }); if (tool) toolId = tool.id; }
  ...
  } catch (err) { console.error('Create assessment error:', err); return res.status(500).json({ error: 'Erro interno' }); }
});
...
userAssessmentRouter.patch('/:id/responses', async (req, res) => {
  try {
    ...
    if (!responses || typeof responses !== 'object' || Array.isArray(responses)) return res.status(400).json({ error: 'Respostas invalidas' });
    const updated = await prisma.assessment.update({ where: { id: assessment.id }, data: { responses }, select: { id: true, responses: true, updatedAt: true } });
    return res.json({ assessment: updated, message: 'Progresso salvo' });
  } catch (err) { console.error('Save responses error:', err); return res.status(500).json({ error: 'Erro ao salvar progresso' }); }
});

userAssessmentRouter.post('/:id/submit', async (req, res) => {
  try {
    const assessment = await prisma.assessment.findUnique({ where: { id: req.params.id }, include: { user: { select: { name: true, email: true } }, report: { select: { id: true, generatedAt: true } } } });
    if (!assessment) return res.status(404).json({ error: 'Assessment nao encontrado' });
    if (assessment.userId !== req.user.id) return res.status(403).json({ error: 'Sem permissao' });
    if (assessment.status !== 'IN_PROGRESS') return res.status(400).json({ error: 'Assessment ja foi submetido' });
    ...
    const completed = await prisma.assessment.update({ where: { id: assessment.id }, data: { responses, scoresRaw: scoresData, profilePrimary, profileSecondary, status: 'COMPLETED', completedAt: new Date() }, include: { report: { select: { id: true, generatedAt: true } } } });
    if (autoGenerateReportTools.has(toolSlug) && !completed.report) setImmediate(() => generateReportInBackground({ assessmentId: assessment.id, handler, userEmail: assessment.user.email, userName: assessment.user.name }));
  ...
  } catch (err) { console.error('Submit assessment error:', err); return res.status(500).json({ error: err.message || 'Erro interno' }); }
});

userAssessmentRouter.get('/mine', async (req, res) => {
  try {
    const assessments = await prisma.assessment.findMany({ where: { userId: req.user.id }, select: { id: true, status: true, responses: true, profilePrimary: true, profileSecondary: true, scoresRaw: true, completedAt: true, releasedAt: true, createdAt: true, tool: { select: { slug: true, name: true } }, report: { select: { id: true, generatedAt: true } } }, orderBy: { createdAt: 'desc' } });
    return res.json({ assessments });
  } catch (err) { console.error('List my assessments error:', err); return res.status(500).json({ error: 'Erro interno' }); }
});
...
userAssessmentRouter.get('/:id/report', async (req, res) => {
  ...
  } catch (err) { console.error('Get report error:', err); return res.status(500).json({ error: 'Erro interno' }); }
});

userAssessmentRouter.get('/evolution', async (req, res) => {
  try {
    const assessments = await prisma.assessment.findMany({ where: { userId: req.user.id, status: { in: ['COMPLETED', 'REVIEWED', 'RELEASED', 'REPORT_GENERATED'] } }, select: { id: true, scoresRaw: true, profilePrimary: true, profileSecondary: true, completedAt: true, createdAt: true }, orderBy: { completedAt: 'asc' } });
    return res.json({ assessments });
  } catch (err) { console.error('Evolution error:', err); return res.status(500).json({ error: 'Erro interno' }); }
});
...
adminAssessmentRouter.get('/', async (req, res) => {
  try {
    ...
    const pageNum = Math.max(1, parseInt(page)); const limitNum = Math.min(100, Math.max(1, parseInt(limit))); const skip = (pageNum - 1) * limitNum; const where = {};
    if (status) where.status = status;
    if (toolSlug) { const toolRecord = await prisma.tool.findUnique({ where: { slug: toolSlug } }); if (toolRecord) where.toolId = toolRecord.id; }
    const [assessments, total] = await Promise.all([prisma.assessment.findMany({ where, select: { id: true, status: true, profilePrimary: true, profileSecondary: true, scoresRaw: true, adminNotes: true, completedAt: true, releasedAt: true, createdAt: true, tool: { select: { slug: true, name: true } }, user: { select: { id: true, name: true, email: true } }, report: { select: { id: true, generatedAt: true } } }, orderBy: { createdAt: 'desc' }, skip, take: limitNum }), prisma.assessment.count({ where })]);
